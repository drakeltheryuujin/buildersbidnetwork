      ...
      if uid.present? && provider.present?
        ...
        if auth.present?
          auth.update_attributes(
              ...
              :avatar_url => (avatar_url.blank? ? auth.avatar_url : avatar_url)) unless token.blank? && secret.blank? && avatar_url.blank?
          ...
          if user_signed_in? # link accounts
            current_user.authentications.create(:provider => provider, :uid => uid, :token => token, :secret => secret, :avatar_url => avatar_url)
            ...
            session["devise.omniauth_authentication"] = {:provider => provider, :uid => uid, :token => token, :secret => secret, :avatar_url => avatar_url}
